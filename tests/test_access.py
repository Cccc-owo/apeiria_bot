...
def test_evaluate_superuser_bypasses_deny() -> None:
    ac = _control([_rule("user", "boss", "deny")])
    assert ac.evaluate("boss", None, "p", is_superuser=True) is True


def test_evaluate_no_matching_rule_allows() -> None:
    assert _control([]).evaluate("u1", None, "p") is True


def test_evaluate_user_allow() -> None:
    ac = _control([_rule("user", "u1", "allow")])
    assert ac.evaluate("u1", None, "p") is True


def test_evaluate_user_deny() -> None:
    ac = _control([_rule("user", "u1", "deny")])
    assert ac.evaluate("u1", None, "p") is False


def test_evaluate_group_deny_only_matching_group() -> None:
...
def test_evaluate_higher_priority_wins() -> None:
    ac = _control(
        [
...
def test_evaluate_plugin_scoped_rule() -> None:
...
def test_evaluate_none_plugin_applies_to_all() -> None:
...
def _event(user_id):
    return SimpleNamespace(get_user_id=lambda: user_id)
...
async def test_access_preprocessor_blocks_on_deny(monkeypatch) -> None:
    ...
    async def _not_superuser(_bot, _event) -> bool:
        return False

    monkeypatch.setattr(hook, "SUPERUSER", _not_superuser)
    monkeypatch.setattr(hook, "check_access", lambda *_a, **_kw: False)
    matcher = SimpleNamespace(plugin_name="repeater")
    with pytest.raises(IgnoredException):
        await hook.access_preprocessor(
            matcher, _bot(), _event("u1"), _session("u1", is_group=False)
        )
...
async def test_access_preprocessor_passes_on_allow(monkeypatch) -> None:
    ...
    monkeypatch.setattr(hook, "SUPERUSER", _not_superuser)
    monkeypatch.setattr(hook, "check_access", lambda *_a, **_kw: True)
    matcher = SimpleNamespace(plugin_name="repeater")
    await hook.access_preprocessor(
        matcher, _bot(), _event("u1"), _session("u1", is_group=False)
    )
...
async def test_access_preprocessor_superuser_bypasses(monkeypatch) -> None:
    from apeiria.access import hook

    async def _is_superuser(_bot, _event) -> bool:
        return True

    monkeypatch.setattr(hook, "SUPERUSER", _is_superuser)
    monkeypatch.setattr(
        hook,
        "check_access",
...
def test_check_access_superuser_passes(monkeypatch) -> None:
    ...
    ac = _control([_rule("user", "u1", "deny")])
    monkeypatch.setattr("apeiria.bootstrap.steps.get_access_control", lambda: ac)
    assert hook.check_access("p", "u1", None, is_superuser=True) is True
