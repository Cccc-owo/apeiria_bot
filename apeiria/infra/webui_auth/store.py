...
def list_accounts() -> list[HostWebUIAccount]:
    ...
    return [
        HostWebUIAccount(
            ...
            last_login_at=(
                str(item.get("last_login_at")) if item.get("last_login_at") else None
            ),
            password_changed_at=(
                str(item.get("password_changed_at"))
                if item.get("password_changed_at")
                else None
            ),
        )
        for item in _users(data)
...
def create_registration_code(
    *,
    ...
    payload = _new_registration_code(role=normalized_role, created_by=created_by)
    ...
    _persist_auth_store(data)
    return HostRegistrationCode(**payload)
...
def set_account_disabled(username: str, *, disabled: bool) -> str:
    ...
    if (
        disabled
        and str(user.get("role") or "") == "owner"
        and not bool(user.get("is_disabled", False))
...
def delete_account(username: str) -> str:
    ...
    if user is None:
        raise ValueError("account_not_found")
    if (
        str(user.get("role") or "") == "owner"
        and not bool(user.get("is_disabled", False))
        and _enabled_owner_count(data) <= 1
    ):
        raise ValueError("last_owner_forbidden")
    normalized_username = str(user.get("username") or "")
    data["users"] = [
        item
        for item in _users(data)
        if str(item.get("username") or "") != normalized_username
