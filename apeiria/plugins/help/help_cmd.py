...
async def _show_help_list(bot: Bot, event: Event) -> None:
    ...
    if _is_console(bot):
        await _help.finish(_format_help_list_text(plugins))
    ...
    try:
        ...
        logger.opt(exception=exc).warning(
            "Help list image render failed, fallback to text."
        )
        await _help.finish(_format_help_list_text(plugins))
...
async def _show_plugin_detail(bot: Bot, name: str) -> None:
    ...
    if _is_console(bot):
        await _help.finish(_format_plugin_detail_text(plugin_info))
